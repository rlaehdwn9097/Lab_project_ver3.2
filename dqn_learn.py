...
# !       self.MicroBS_AR = self.get_AR("MicroBS")
    ...
    def train(self, max_episode_num):
        ...
        for ep in range(int(max_episode_num)):
            ...
            while not done:
                ...
                if self.buffer.buffer_count() > cf.MAX_ROUNDS * 0.05:  # start train after buffer has some amounts
            ...
            '\taction_cnt: ',self.action_cnt,'\tcache_hit: ', self.cache_hit_cnt, '\tcache_hit_rate: ', cache_hit_rate, '\t','Reward: ', episode_reward, '\t','Redundancy: ', redundancy, '\n')

            self.write_result_file( ep, time, self.last_round_nb, self.step_cnt, self.action_cnt, self.cache_hit_cnt, cache_hit_rate, episode_reward, redundancy)
    ...
    def plot_cache_hit_result(self):
        plt.plot(self.save_epi_cache_hit_rate)
        plt.savefig('cache_hit_rate.png')
        plt.show()

    def plot_redundancy_result(self):
        plt.plot(self.save_epi_redundancy)
        plt.savefig('redundancy.png')
        plt.show()

    def write_result_file(self, ep, time, NB_Round, step_cnt, action_cnt, cache_hit, cache_hit_rate, episode_reward, redundancy):
